      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {benefits.map((benefit) => (
            <div key={benefit.title} className="bg-gray-dark rounded-xl p-6 border border-gray-light">
              <div className="text-3xl mb-3">{benefit.icon}</div>
              <h3 className="text-lg font-bold text-gray-text mb-2">{benefit.title}</h3>
              <p className="text-gray-muted text-sm leading-relaxed">{benefit.description}</p>
            </div>
          ))}
        </div>

        {/* CTA Section */}
        <div className="bg-gray-dark rounded-xl p-8 sm:p-12 border border-gray-light text-center">
          <h2 className="text-2xl font-bold text-gray-text mb-4">
            Ready to Find Your Optimal Hunt?
          </h2>
          <p className="text-gray-muted mb-8 max-w-xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Link
              href="/signup"
              className="border border-amber-brand text-amber-brand px-6 py-3 rounded-lg font-semibold hover:bg-amber-brand hover:text-gray-dark transition-colors"
            >
              List Your Business
            </Link>
          </div>
